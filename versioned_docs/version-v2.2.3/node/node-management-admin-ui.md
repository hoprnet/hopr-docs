...
| **Listen host** | The entry point of the session consists of the internal IP address of the HOPRd Docker container and the exposed listening port. Example: **`0.0.0.0:1422`**. <br/><br/>To use this, you must run a HOPRd node and expose both **UDP** and **TCP** ports (e.g., **1422**) to receive response traffic. If you specify only the port (e.g., **`:1422`**), the system will automatically resolve the internal IP address of the Docker container.|
