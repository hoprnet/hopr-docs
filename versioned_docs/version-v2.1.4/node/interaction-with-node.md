...
Before proceeding, ensure your HOPR node is set up. If you haven’t done so yet, please start with the setup guide [here](./run-a-node-overview.md).
...
(**1**) Navigate to the "**PEERS**" page where you can see list of peers (nodes) which are known to your node. Please select a random peer where the connection quality under the "**Quality**" column is greater than **50%**. Click the "**PING node**" icon. If the ping is successful, you will receive a response time in milliseconds. If the ping fails, you'll see the message: "**Ping of \<PeerID> failed**", where "**\<PeerID>**" represents the selected node's peer ID.
...
(**2**) Once you’ve successfully pinged a node, you can assign it an alias (i.e., a custom name). On the same "**PEERS**" page, locate the successfully pinged node and click the "**ADD new alias**" icon. In the popup window, enter the desired name under the "**Alias**" field (e.g., **Betty**) and click "**Add**".
...
(**1**) There are two ways to open a payment channel with a peer which has alias **Betty**:

- On the "**PEERS**" page, next to the selected peer, click on "**OPEN outgoing channel**". In the popup window, enter the amount of wxHOPR you would like to allocate for the payment channel (e.g., **1**) and click "**Open Channel**".
...
(**2**) In a few minutes this will open a channel from you to Betty with 1 wxHOPR staked in it and you will receive a message about successfully opened channel. You can use these tokens to pay Betty and all the other nodes in any relay you use, where Betty is the first intermediary node. All your active outgoing channels can be viewed on the "**CHANNELS: OUT**" page.

**Note**: Channels are unidirectional; opening this channel does not mean a channel from Betty to your node exists. Only one channel can exist in a single direction between two nodes. 
...
There are two methods for sending messages: direct messaging and HOP messaging, which involves using other peers as relays.
...
Let's start by sending a direct (0-HOP) message to the **Betty's** node:

(**1**) Go to the "**MESSAGES**" page, and at the top, click on "**SEND message**" icon. In the popup, fill in the following:
...
(**2**) Click on "**Send**". The **Betty** node should now receive your message.

### Sending a 1-HOP message via Betty's node

Sending a direct (0-HOP) message, as we did earlier, does not provide privacy or encryption because it is not using intermediate nodes to relay data. Now, let's secure the message by sending it through the mixnet using a 1-HOP relay. Since you already have an open payment channel with **Betty**, we will use her node as an intermediary to send the message back to yourself, ensuring the message is private and secure.
...
This means to send 2-HOP message to yourself through "**Betty**" and "**Chao**": **me → Betty → Chao → me** would require channels to be open from **me → Betty** and from **Betty → Chao**. The final channel from **Chao → me** is not required as the last HOP of a relay is not incentivised. We assume that the reciever has an inherant desire to receive messages.
...
So far, we have used manually selected paths by entering the whole path. Instead of this, we can instead let HOPR find a path for the relay by specifying just the receiver. Automatic pathing will only look for **3-HOP paths** (**three intermediate nodes**) from you to the receiver. If none exist or you don’t have sufficient funds staked in the first channel of the relay, it will fail.
...
(**2**) Click "**Send**". You should now receive the message, relayed through automatically found path via three intermediate nodes. You can view all received messages on the "**MESSAGES**" page.
...
(**1**) Make sure you don't have opened payment channel with "**Chao's**" node. If you do please follow [here](#5-close-outgoing-or-incoming-channels) on how to close it.
...
#### Relaying a message through a path where there is no open payment channel between one of the intermediate nodes
</summary>
...
Message can be relayed only via derection of the established path. You won’t be able to make in this case 2-HOP message in the other direction as you don’t have an open channel with **Chao**. And **Chao** may not have an open channel with **Betty**.
...
In the established path you can not have consecutively repeating nodes. For example: **me → Betty → Betty → Zoe**.
...
In this section we will learn on how to close outgoing & incoming channels.

### Close outgoing channel with Chao node

To close a payment channel, two actions are required. The first step is to initiate the channel closure, and the second is to finalize the closure after the grace period has ended.
...
(**2**) After receiving the message indicating the closure is pending, wait for the **5-minute grace period**. The grace period allows the counterparty node time to redeem tickets on their end.
...
Navigate to "**TICKETS**" page, where you will see tickets statistics:

- **Unredeemed value**: Unredeemed tickets means your node received tickets for relaying data via your node and they are not being redeemed into wxHOPR tokens.
...
- **Redeemed value**: Unredeemed tickets which have been aggregated and redeemed into wxHOPR. After redemption process wxHOPR tokens are bing sent directly to your [HOPR Safe wallet](../token/safestaking.md).