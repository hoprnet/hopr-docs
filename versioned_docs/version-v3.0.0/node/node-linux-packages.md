...
Below are the key file locations used by the HOPRd installation:
...
| HOPRd binary        | `/usr/bin/hoprd` | Main `hoprd` executable compiled in Rust                           |
| Hopli CLI tool      | `/usr/bin/hopli` | `hopli` command-line tool for interacting with the node            |
