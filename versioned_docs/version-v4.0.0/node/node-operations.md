...
| Data directory | `$HOME/.hoprd-db-dufour/` | Contains the node’s database (`node_db` folder). |
...
2. Remove the container using the following command: `docker rm -f <Your_Node_Container_ID>`. Replace `<Your_Node_Container_ID>` with your container ID.
